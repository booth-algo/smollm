...
# Script to find a free GPU and run train or test script on it
# Usage: ./run.sh [train|test] [additional args...]
# This script checks GPU memory usage and runs on the least utilized GPU
...
if [[ "$MODE" != "train" && "$MODE" != "test" ]]; then
    ...
    echo ""
    echo "Examples:"
    echo "  $0 train          # Run training script"
    echo "  $0 test           # Run testing/profiling script"
    exit 1
fi
...
echo ""
echo "Starting $MODE on GPU $GPU_ID..."
